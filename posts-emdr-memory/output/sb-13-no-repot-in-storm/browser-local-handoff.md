...
curl -fsS -X POST "http://195.209.210.45:8787/publish" \
  ...
  -d '{"topic":"sb-13-no-repot-in-storm"}'
```
...
python3 scripts/fetch-topic-cover.py --topic sb-13-no-repot-in-storm
python3 scripts/publish-browser-deferred.py --topic sb-13-no-repot-in-storm --submit --finish --git-push
```
