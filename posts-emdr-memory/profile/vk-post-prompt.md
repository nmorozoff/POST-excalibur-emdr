...
ВК **не поддерживает** markdown `[текст](url)`, HTML `<a href>` и markdown-жирный `**текст**`.
Сырые `**` в ленте видны как звёздочки — это баг контента/пайплайна.

### Заголовок

- Первая строка тела = заголовок.
- В `.md` можно писать `**Заголовок**` для удобства в редакторе.
- При публикации `format_vk_publish_text()`:
  - кириллица → **без** `**` (plain title; в Unicode нет math-bold для кириллицы);
  - латиница/цифры внутри `**` → Mathematical Sans-Serif Bold (визуально жирный в ленте).
- **Не** оставлять видимые `**` в `message` для MCP.
