...
**Симптом:** в посте Макс фраза «напишите в личных сообщениях» открывает **пост канала про EMDR**, а не чат с ботом.

**Причина:** в реестре ошибочно стоял URL поста `.../AZ9H9rFePFc` (статья про EMDR), а не бот канала `https://max.ru/id771605638595_bot`. Плюс дубли `[ТУТ]` в markdown.

**Правильно:** ЛС Макс = `get_max_ls_url()` → `https://max.ru/id771605638595_bot` (бот комментариев/лички канала). Уникальные якоря в тексте. Автофикс: `fix_max_markdown_links()`.
